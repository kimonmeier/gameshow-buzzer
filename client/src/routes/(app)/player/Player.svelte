<script lang="ts">
	...
	onMount(() => {
		inputs.subscribe((x) => {
			tick().then(() => {
				const info = x.find((x) => x.playerId == player.id)!;
...
<div class="flex flex-col">
	<div class="flex flex-row items-center justify-center">
		<button
			class="rounded-full {buzzerBackgroundColor()} mr-4 w-40 h-40 justify-center flex items-center"
			on:click={buzzer}
		>
			{#if isBuzzerLocked}
				{#if myBuzzerWasFirst}
		...
		<div class="flex flex-col w-3/4 gap-2">
			<input
				type="text"
				class="row-start-2 bg-slate-300 rounded-xl mt-10 px-2 py-1 text-black font-mono"
				disabled={inputInfo.isLocked}
			...
			{:else}
				<button class="bg-indigo-600 row-start-4 rounded-xl py-1" on:click={lockInput}
					>Bestätigen</button
				>
			{/if}
		</div>
